...
> 项目的架构设计、开发过程、踩坑记录和日常运维方法见 **[维护手册.md](维护手册.md)**。
...
- **人人对战**：
  ...
  - 认输、求和、再来一局（自动换边）
- **完整规则**：将军 / 绝杀 / 困毙（无子可动判负）/ 白脸将（王不见王）/ 蹩马腿 / 塞象眼 / 过河兵横走 / **长将判负**（连续将军逼重复局面判该方负，不是简单和棋）；无将军的重复局面仍判和
- **计时**：5 / 10 / 15 分钟包干 + 每步 3 秒加秒，超时判负；联机模式由服务器计时
- 中文着法记谱（如「炮二平五」）、最后一步高亮、将军提示、移动端自适应
- **意见反馈**：玩家可在主页或对局页提交游戏体验/建议，服务器通过 Gmail 转发到站长邮箱（收件地址仅存于服务器环境变量，不出现在代码中）
